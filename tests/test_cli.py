...
def test_slug_helper():
    assert main._slug("Tesla Model S") == "tesla_model_s"
    assert main._slug("iPhone 15!!") == "iphone_15"
    assert main._slug("   ") == "listings"


def test_main_default_output_filename_is_query_slug(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(main, "scrape", lambda *a, **kw: _fake_result())
    rc = main.main(["--query", "Tesla Model S"])
    assert rc == 0
...
def test_main_custom_output_filename(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(main, "scrape", lambda *a, **kw: _fake_result())
    main.main(["--query", "Tesla Model S", "--out", "custom"])
...
def test_run_cli_playwright_error_exits_1(monkeypatch, capsys):
    monkeypatch.setattr(main, "scrape", lambda *a, **kw: (_ for _ in ()).throw(PlaywrightError("net down")))
    rc = main.run_cli(["--query", "Tesla"])
    assert rc == 1
...
def test_run_cli_keyboard_interrupt_exits_130(monkeypatch):
    monkeypatch.setattr(main, "scrape", lambda *a, **kw: (_ for _ in ()).throw(KeyboardInterrupt()))
    assert main.run_cli(["--query", "Tesla"]) == 130


def test_run_cli_success_returns_0(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(main, "scrape", lambda *a, **kw: _fake_result())
    assert main.run_cli(["--query", "Tesla"]) == 0
